...
function normalizeCountryName(countryName) {
  ...
  return normalizedCountryName === '' ? null : normalizedCountryName;
}
...
const COUNTRY_NAME_TO_ID = Object.freeze(
  Object.fromEntries(
    Object.entries(COUNTRY_NAMES).map(([countryId, countryName]) => [
...
const COUNTRY_LOOKUP = Object.freeze({
  ...COUNTRY_NAME_TO_ID,
  ...COUNTRY_ALIASES,
});
...
function getCountryName(countryId) {
  ...
  return COUNTRY_NAMES[normalizedCountryId] ?? null;
}
...
function getCountryIdByName(countryName) {
  ...
  return COUNTRY_LOOKUP[normalizedCountryName] ?? null;
}
...
function findCountryIdInText(text) {
  ...
  for (const match of normalizedText.matchAll(/\bfrom\s+([a-z0-9\s]+)/g)) {
    const tokens = match[1].trim().split(/\s+/).filter(Boolean);

    for (let length = tokens.length; length > 0; length -= 1) {
      const candidate = tokens.slice(0, length).join(' ');
      const countryId = getCountryIdByName(candidate);
